...
func Run(app *fiber.App, db *gorm.DB) {
	...
	userShopRepo := usershops.NewRepository(db)
	...
	marketplaceService := marketplace.NewService(marketplaceClient, marketplaceRepo, userShopRepo, config.App.RedirectURL)
